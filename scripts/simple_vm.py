...
class Machine:
    ...
    def dispatch(self, op):
        ...
        if op in dispatch_map:
            ...
            self.push(op[1:-1])  # push quoted strings to stack
        else:
...
def parse(text):
    ...
    for toknum, tokval, _, _, _ in tokens:
        if toknum == tokenize.NUMBER:
            ...
            raise RuntimeError('Unknown tokens {}: "{}"'.format(tokenize.tok_name[toknum], tokval))
    ...
    while True:
        ...
        for i, (a, b, op) in enumerate(zip(code, code[1:], code[2:])):
            if isinstance(a, int) and isinstance(b, int) and op in {'+', '-', '*', '/'}:
                ...
                m.run()
                code[i:i + 3] = [m.top()]
                print('Constant-folded {}{}{} to {}'.format(a, op, b, m.top()))
                break
        else:
            break
    return code
...
def repl():
    print('Hit CTRL+D to type "exit" to quit.')
    while True:
        try:
            source = get_input('> ')
...
def test(code=None):
    ...
    print('Result: {}'.format('OK' if result else 'FAIL'))
    return result
